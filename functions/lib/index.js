...
*/
//Imports
const Signup_1 = require("./classes/authentication/Signup");
const Login_1 = require("./classes/authentication/Login");
const FetchData_1 = require("./classes/data/FetchData");
const ResturentFunctions_1 = require("./classes/resturents/ResturentFunctions");
        ...
        if (snapshot.size > 0) {
            ...
            return users.where('userName', '==', req.body.userName).get().then((foundUser) => {
                if (foundUser.size > 0) {
                    ...
                    return Signup_1.default.register(db, data).then(signedUpData => {
                        console.log("REGISTER ::: REQ DONE :" + req.body.userName);
                        if (signedUpData) {
                            return Login_1.default.saveAuthInfo(users, signedUpData, data.phone).then(usr => {
                                if (usr) {
...
exports.login = functions.https.onRequest((req, res) => {
    ...
    return Login_1.default.login(users, data).then(user => {
        let sendData;
        if (user.size === 1) {
            ...
            return Login_1.default.saveAuthInfo(users, sendData, data.phone).then(usr => {
                if (usr) {
...
exports.changephone = functions.https.onRequest((req, res) => {
    const data = req.body;
    if (data.phone) {
        return users.where('phone', '==', data.phone).get().then(phoneFound => {
            if (phoneFound.size === 0) {
                console.log("CHANGE PHONE ::: Auth : " + data.userName);
                return Login_1.default.saveAuthInfo(users, { userName: data.userName }, data.phone).then(done => {
                    if (done) {
...
exports.fetchdata = functions.https.onRequest((req, res) => {
    ...
    if (data.userName && data.uid) {
        ...
        return FetchData_1.default.fetchData(data, users).then(foundData => {
            if (foundData) {
...
exports.registerResturent = functions.https.onRequest((req, res) => {
    corsHandler(req, res, () => {
        console.log("cors added");
    });
    if (req.body.userName) {
        return ResturentFunctions_1.default.register(resturents, users, enterprises, req, res);
    }
    else {
        return null;
    }
});
exports.loginResturent = functions.https.onRequest((req, res) => {
    corsHandler(req, res, () => {
        console.log("cors added");
    });
    if (req.body.userName) {
        return ResturentFunctions_1.default.loginWithUserName(resturents, req, res);
    }
    else {
        return null;
    }
